const exec = (folder) => {
    ...
    let pack = fs.readFileSync(`${cwd}/package.json`);
    if (!pack) {
    ...
    fs.writeFileSync(`${cwd}/package.json`, pack);
    


    shell.echo('Project created, copying support files');
    shell.cp(`${__dirname}/../support/__script.js`, './__script.js');
    shell.cp(`${__dirname}/../support/__versioning.js`, './__versioning.js');
    ...
    shell.rm('-rf', 'sample-react-app');
    shell.echo('Done.');
    shell.echo('Installing rimraf.');
    shell.exec('npm i rimraf');
    shell.echo('Finished!. Open your folder and start developing!');
